...
apiClient.serviceReady().then(async (params) => {
	...
	let toRun: string | undefined =  '/workspace/my-script.php';
	...
	const phpRoot = URI.file(`/home/dirkb/bin/wasm`);
	devices.push({ kind: 'fileSystem', uri: phpRoot, mountPoint: path.posix.sep });
	const wasi = WASI.create('php-cgi', apiClient, exitHandler, devices, params.stdio, {
		args: toRun !== undefined ? [toRun] : []
	});
	const binary = apiClient.vscode.workspace.fileSystem.readFile(phpRoot.with({ path: path.join(phpRoot.path, 'php-cgi.wasm') }));
