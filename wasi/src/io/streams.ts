...
export namespace streams {
	...
	export namespace $cm {
		...
		export namespace $ {
			const allFunctions = [$read, $blocking_read, $skip, $blocking_skip, $subscribe_to_input_stream, $drop_input_stream, $write, $blocking_write, $write_zeroes, $blocking_write_zeroes, $splice, $blocking_splice, $forward, $subscribe_to_output_stream, $drop_output_stream];
			export function createHost<T extends $wcm.Host>(service: streams, context: $wcm.Context): T {
				return $wcm.Host.create<T>(allFunctions, service, context);
			}
			export function createService<T extends streams>(wasmInterface: $wcm.WasmInterface, context: $wcm.Context): T {
